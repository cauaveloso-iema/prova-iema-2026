...
Detecção real de círculos preenchidos no cartão-resposta
"""
...
class OMRReader:
    ...
    def analyze_bubble(self, bubble, binary_image):
        ...
        Retorna percentual de preenchimento (0 a 1)
        """
    ...
    def detect(self, image_base64=None):
        ...
        if self.debug:
            cv2.imwrite(f"{self.debug_folder}/0_original_resized.jpg", image)
            print(f"[OMR] Imagem redimensionada: {image.shape[1]}x{image.shape[0]}")
        ...
        # Salvar imagem de debug com resultados
        if self.debug:
...
def detect():
    try:
        ...
        # Atualizar threshold se enviado
