...
// ============================================
// CLICAR NO ALUNO PROCESSA DIRETO
// ============================================
async function selecionarAluno(alunoId, alunoNome, itemEl) {
    ...
    if (itemEl) {
        ...
        itemEl.style.borderColor = '#8b5cf6';
        itemEl.style.pointerEvents = 'none';
        itemEl.innerHTML = `
            <div>
                <strong>${escapeHTML(alunoNome)}</strong>
                <br>
                <small style="color: #7c3aed;">Processando...</small>
            </div>
...
function renderizarCamposEspecificos(tipo) {
    ...
    const camposComuns = `
        ...
        <div class="mb-3">
            <label class="form-label">Condição Social</label>
            <textarea id="detalheCondicaoSocial" class="form-control" rows="2" placeholder="Informações sobre a condição social do aluno..."></textarea>
        </div>
    `;
    ...
    switch (tipo) {
            ...
            html = `
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" id="detalheTipoIntervencao" class="form-control" placeholder="Ex: Individual, Familiar, Grupo">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Duração (minutos)</label>
                        <input type="number" id="detalheDuracaoSessao" class="form-control" min="1">
                    </div>
                </div>
                <div class="mb-3">
                    <label class="form-label">Métodos Utilizados</label>
                    <textarea id="detalheMetodosUtilizados" class="form-control" rows="2" placeholder="Técnicas e métodos aplicados..."></textarea>
                </div>
                ${camposComuns}
            `;
            break;
            ...
            html = `
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="detalheModalidadeAtendimento" class="form-select">
                            <option value="">Selecione...</option>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="detalheParticipantesAtendimento" class="form-control" placeholder="Ex: Mãe, Aluno, Professor">
                </div>
                ${camposComuns}
            `;
            break;
        ...
        default:
            // Evasão, Desinteresse, Problemas com Disciplina, Insubordinação, Vulnerabilidade às Drogas, Agressividade, Baixo Rendimento
            html = camposComuns;
            break;
    }
...
function coletarDetalhes() {
    ...
    const participantesEl = safeGet('detalheParticipantesAtendimento');
    if (participantesEl) {
        detalhes.participantesAtendimento = participantesEl.value
            .split(',')
            .map(t => t.trim())
            .filter(t => t.length > 0);
    }
    ...
    const camposTexto = {
        ...
        'detalheCondicaoSocial': 'condicaoSocial',
        ...
        'detalheMetodosUtilizados': 'metodosUtilizados',
        'detalheModalidadeAtendimento': 'modalidadeAtendimento',
    ...
    const dataEl = safeGet('detalheAgendadoPara');
    if (dataEl && dataEl.value) detalhes.agendadoPara = new Date(dataEl.value);
    
    const duracaoEl = safeGet('detalheDuracaoSessao');
    if (duracaoEl && duracaoEl.value) {
        const d = parseInt(duracaoEl.value);
        if (!isNaN(d) && d > 0) detalhes.duracaoSessao = d;
    }
...
async function carregarDashboard() {
    try {
        const response = await fetch('/api/assistente-social/dashboard', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
function inicializarAutocompleteAluno() {
    ...
    document.addEventListener('click', (e) => {
        const listEl2 = safeGet('autocompleteAlunoList');
        const input2 = safeGet('buscaAlunoRelatorio');
        
        if (!listEl2 || !input2) return;
        if (listEl2.style.display === 'none') return;
        
        if (listEl2.contains(e.target) || input2.contains(e.target)) {
            return;
        }
...
async function carregarAlunosParaRelatorio() {
    ...
    try {
        const turmasRes = await fetch('/api/assistente-social/turmas', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        for (const turma of turmasData.turmas) {
            try {
                const res = await fetch(`/api/assistente-social/alunos-por-turma?turma=${encodeURIComponent(turma)}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                        ...
                        <div class="row">
                            ${porTipo.map(t => `
                                <div class="col-md-4 mb-2">
                                    <div class="p-2" style="background:#ede9fe;border-radius:8px;">
...
function exportarCSV() {
    ...
    link.download = `assistente-social_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
