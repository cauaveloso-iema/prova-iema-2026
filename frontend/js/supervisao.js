...
async function selecionarAluno(alunoId, alunoNome, itemEl) {
    ...
    if (itemEl) {
        itemEl.style.background = '#e0f2fe';
        itemEl.style.borderColor = '#0ea5e9';
        itemEl.style.pointerEvents = 'none';
        itemEl.innerHTML = `
            <div>
                <strong>${escapeHTML(alunoNome)}</strong>
                <br>
                <small style="color: #0284c7;">Processando...</small>
            </div>
...
async function carregarDashboard() {
    try {
        const response = await fetch('/api/supervisao/dashboard', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function carregarAlunosParaRelatorio() {
    ...
    try {
        const turmasRes = await fetch('/api/supervisao/turmas', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        for (const turma of turmasData.turmas) {
            try {
                const res = await fetch(`/api/supervisao/alunos-por-turma?turma=${encodeURIComponent(turma)}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                        ...
                        <div class="row">
                            ${porTipo.map(t => `
                                <div class="col-md-4 mb-2">
                                    <div class="p-2" style="background:#e0f2fe;border-radius:8px;">
