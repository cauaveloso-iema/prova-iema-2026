...
class MonitoramentoTempoReal {
  ...
  configurarEventosFiltros() {
    ...
    if (tipoSelect) {
      ...
      console.log('✅ Evento configurado no select Tipo');
    }
    ...
    if (dataSelect) {
      ...
      console.log('✅ Evento configurado no select Data');
    }
    ...
    if (turmaSelect) {
      ...
      console.log('✅ Evento configurado no select Turma');
    }
    ...
    if (turmaRefeicaoSelect) {
      ...
      console.log('✅ Evento configurado no select Turma (Refeições por Turma)');
    }
    ...
    if (periodoRefeicaoSelect) {
      ...
      console.log('✅ Evento configurado no select Período (Refeições por Turma)');
    }
    ...
    if (rodizioTurmaSelect) {
      ...
      console.log('✅ Evento configurado no select Turma (Rodízio)');
    }
    ...
    if (rodizioTipoSelect) {
      ...
      console.log('✅ Evento configurado no select Tipo (Rodízio)');
    }
  ...
  // ============================================
  // FILTROS DE REFEIÇÕES POR TURMA (CORRIGIDO)
  // ============================================
  ...
  aplicarFiltrosRefeicoesTurmas() {
    ...
    console.log(`📌 Filtros Turmas: Turma=${turma}, Período=${periodo}`);
    ...
    if (periodo !== 'todas') {
      dadosFiltrados = dadosFiltrados.map(t => ({
        turma: t.turma,
        manha: periodo === 'manha' ? t.manha : 0,
        almoco: periodo === 'almoco' ? t.almoco : 0,
        tarde: periodo === 'tarde' ? t.tarde : 0,
        total: t[periodo] || 0,
              ...
              <table class="data-table">
                ...
                <tbody>
                  ...
                  ${(cozinha.refeicoesPorTurma || []).length === 0 ? `
                    <td><td colspan="6" class="text-center empty-state">Nenhum registro hoje<\/td></tr>
                  ` : ''}
                </tbody>
              </table>
            </div>
