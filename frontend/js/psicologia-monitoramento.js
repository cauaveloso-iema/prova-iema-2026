// ============================================
// MONITORAMENTO PSICOLOGIA - CLASSE PRINCIPAL
// Tema: Verde-Água / Teal
// ============================================

class MonitoramentoPsicologia {
    constructor() {
        this.apiBase = '/api/psicologia';
        this.atendimentos = [];
    ...
    async carregar() {
        console.log('🧠 Carregando Monitoramento Psicologia...');
    ...
    renderLoading() {
        return `
            <div style="text-align: center; padding: 60px; background: white; border-radius: 12px;">
                <div style="width: 50px; height: 50px; border: 5px solid #f3f3f3; border-top: 5px solid #14b8a6; border-radius: 50%; margin: 0 auto 20px; animation: spin 1s linear infinite;"></div>
                <p style="color: #6b7280;">Carregando dados de Psicologia...</p>
            </div>
    ...
    renderErro(mensagem) {
        return `
            <div style="text-align: center; padding: 60px; background: white; border-radius: 12px;">
                ...
                <button onclick="monitoramentoPsicologia.carregar()" style="
                    background: #14b8a6;
                    color: white;
                    border: none;
                    padding: 10px 30px;
    ...
    renderPrincipal() {
        ...
        return `
            <div class="psicologia-monitoramento">
                <div class="monitoring-header">
                    <div class="header-left">
                        ...
                        <div class="header-text">
                            <h1>🧠 Psicologia - Monitoramento</h1>
                            <p>Ocorrências, atendimentos e acompanhamento psicológico</p>
                        </div>
                    </div>
                    
                    <div class="header-actions">
                        <button class="btn-refresh" onclick="monitoramentoPsicologia.atualizarDados()">
                            <i class="fas fa-sync-alt"></i> Atualizar
                        </button>
                        <button class="btn-export" onclick="monitoramentoPsicologia.exportarCSV()">
                ...
                <div class="stats-grid">
                    <div class="stat-card primary" onclick="monitoramentoPsicologia.filtrarPorStatus('todos')">
                    ...
                    <div class="stat-card success" onclick="monitoramentoPsicologia.filtrarPorStatus('finalizado')">
                    ...
                    <div class="stat-card warning" onclick="monitoramentoPsicologia.filtrarPorStatus('em_andamento')">
                    ...
                    <div class="filters-grid">
                        ...
                        <div class="filter-group">
                            ...
                            <select id="filtroTipo" class="filter-select">
                                ...
                                <option value="escuta_acolhimento">Escutas de Acolhimento</option>
                                <option value="manejo_crises_emocionais">Manejo de Crises Emocionais</option>
                                <option value="atividades_grupos">Atividades em Grupos</option>
                                <option value="acoes_atividades_saude">Ações e Atividades em Saúde</option>
                                <option value="acoes_socioemocionais_culturais">Ações Socioemocionais e Culturais</option>
                        ...
                        <div class="filter-actions">
                            <button class="btn-filter" onclick="monitoramentoPsicologia.aplicarFiltros()">
                                <i class="fas fa-filter"></i> Filtrar
                            </button>
                            <button class="btn-filter btn-clear" onclick="monitoramentoPsicologia.limparFiltros()">
                ...
                <div class="table-container">
                    ...
                    <div class="pagination-container" id="paginacao">
                        <button class="btn-page" onclick="monitoramentoPsicologia.paginaAnterior()" id="btnAnterior" disabled>‹ Anterior</button>
                        <span class="page-info" id="pageInfo">Página 1 de 1</span>
                        <button class="btn-page" onclick="monitoramentoPsicologia.proximaPagina()" id="btnProxima" disabled>Próxima ›</button>
                    </div>
                </div>
            </div>

            <style>
                .psicologia-monitoramento { padding: 24px; max-width: 1400px; margin: 0 auto; }
                ...
                .btn-export { background: white; color: #14b8a6; }
                ...
                .stat-card.primary .stat-icon { background: linear-gradient(135deg, #14b8a6, #0d9488); }
                ...
                .btn-icon-sm:hover { background: #f3f4f6; color: #14b8a6; }
                ...
                .btn-page:hover:not(:disabled) { background: #f9fafb; border-color: #14b8a6; color: #14b8a6; }
    ...
    renderAlunosReincidentes() {
        ...
        return reincidentes.map(a => `
            <div class="reincidente-item" onclick="monitoramentoPsicologia.filtrarPorAluno('${a._id}')" style="cursor:pointer;">
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-buttons">
                            <button class="btn-icon-sm" onclick="monitoramentoPsicologia.verDetalhes('${a.id}')" title="Ver detalhes">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button class="btn-icon-sm" onclick="monitoramentoPsicologia.editarAtendimento('${a.id}')" title="Editar">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn-icon-sm danger" onclick="monitoramentoPsicologia.excluirAtendimento('${a.id}', '${a.alunoNome}')" title="Excluir">
    ...
    async verDetalhes(atendimentoId) {
        try {
            ...
            // Escuta
            if (d.tipoEscuta) detalhesHTML += `<div class="mb-2"><strong>Tipo de Escuta:</strong> ${d.tipoEscuta}</div>`;
            if (d.duracaoEscuta) detalhesHTML += `<div class="mb-2"><strong>Duração:</strong> ${d.duracaoEscuta} min</div>`;
            
            // Crise
            if (d.tipoCrise) detalhesHTML += `<div class="mb-2"><strong>Tipo de Crise:</strong> ${d.tipoCrise}</div>`;
            if (d.acoesTomadas) detalhesHTML += `<div class="mb-2"><strong>Ações Tomadas:</strong> ${d.acoesTomadas}</div>`;
            if (d.encaminhamentoEmergencial) detalhesHTML += `<div class="mb-2"><strong>Encaminhamento Emergencial:</strong> ${d.encaminhamentoEmergencial}</div>`;
            
            // Grupos
            if (d.nomeAtividade) detalhesHTML += `<div class="mb-2"><strong>Nome da Atividade:</strong> ${d.nomeAtividade}</div>`;
            if (d.participantesAtividade?.length > 0) detalhesHTML += `<div class="mb-2"><strong>Participantes:</strong> ${d.participantesAtividade.join(', ')}</div>`;
            if (d.localAtividade) detalhesHTML += `<div class="mb-2"><strong>Local:</strong> ${d.localAtividade}</div>`;
            if (d.duracaoAtividade) detalhesHTML += `<div class="mb-2"><strong>Duração:</strong> ${d.duracaoAtividade} min</div>`;
            
            // Saúde
            if (d.temaAcao) detalhesHTML += `<div class="mb-2"><strong>Tema da Ação:</strong> ${d.temaAcao}</div>`;
            if (d.publicoAlvo) detalhesHTML += `<div class="mb-2"><strong>Público Alvo:</strong> ${d.publicoAlvo}</div>`;
            if (d.localAcao) detalhesHTML += `<div class="mb-2"><strong>Local:</strong> ${d.localAcao}</div>`;
            
            // Socioemocionais
            if (d.temaSocioemocional) detalhesHTML += `<div class="mb-2"><strong>Tema Socioemocional:</strong> ${d.temaSocioemocional}</div>`;
            if (d.tipoAtividadeCultural) detalhesHTML += `<div class="mb-2"><strong>Tipo Cultural:</strong> ${d.tipoAtividadeCultural}</div>`;
            if (d.parceria) detalhesHTML += `<div class="mb-2"><strong>Parcerias:</strong> ${d.parceria}</div>`;
            
            // Finais
            if (d.providenciasTomadas) detalhesHTML += `<div class="mb-2"><strong>Providências:</strong> ${d.providênciasTomadas}</div>`;
            ...
            modalBody.innerHTML = `
                <div style="padding: 20px; max-height: 70vh; overflow-y: auto;">
                    <div style="text-align: center; margin-bottom: 20px;">
                        <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #14b8a6, #0d9488); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto;">
                    ...
                    <div style="background: #f0fdfa; border-radius: 12px; padding: 15px; margin-bottom: 20px; border: 1px solid #99f6e4;">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                            <div style="text-align: center;">
                                <div style="font-size: 11px; color: #0d9488; margin-bottom: 4px;">ENTRADA</div>
                                <div style="font-size: 13px; font-weight: 600;">${a.entrada.dataHoraFormatada}</div>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 11px; color: #0d9488; margin-bottom: 4px;">SAÍDA</div>
                    ...
                    <div style="background: #f8fafc; border-radius: 12px; padding: 15px; margin-bottom: 15px;">
                        <h4 style="margin: 0 0 10px; font-size: 0.95rem;"><i class="fas fa-align-left" style="color: #14b8a6;"></i> Descrição</h4>
                    ...
                    ${detalhesHTML ? `
                        <div style="background: #ccfbf1; border-radius: 12px; padding: 15px; margin-bottom: 15px; border: 1px solid #5eead4;">
                            <h4 style="margin: 0 0 10px; font-size: 0.95rem; color: #0d9488;"><i class="fas fa-info-circle"></i> Detalhes</h4>
            ...
            this.graficos.tipos = new Chart(ctxTipos, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['#14b8a6', '#0d9488', '#0f766e', '#115e59', '#2dd4bf'],
                        borderRadius: 8
                    }]
                },
    ...
    async exportarCSV() {
        ...
        a.href = url;
        a.download = `psicologia-${new Date().toISOString().slice(0,10)}.csv`;
        a.click();
...
// ============================================
window.MonitoramentoPsicologia = MonitoramentoPsicologia;
window.monitoramentoPsicologia = null;

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        window.monitoramentoPsicologia = new MonitoramentoPsicologia();
    });
} else {
    window.monitoramentoPsicologia = new MonitoramentoPsicologia();
}